...
bool IniParser::_parseBool(std::string &name, std::string &line, std::shared_ptr<IniFile> ini)
{
    ...
    if (isBool)
    {
        Logger::info() << "[INI] boolean value found for property `" << name << "`: " << value << std::endl;
...
bool IniParser::_parseDecimal(std::string &name, std::string &line, std::shared_ptr<IniFile> ini)
{
    ...
    if (state == INTEGRAL)
    {
        int value;
        ss >> value;
        Logger::info() << "[INI] integer value found for property `" << name << "`: " << value << std::endl;
    ...
    if (state == FRACTIONAL || state == EXP || state == EXP_DIGITS)
    {
        double value;
        ss >> value;
        Logger::info() << "[INI] double value found for property `" << name << "`: " << value << std::endl;
...
std::shared_ptr<IniFile> IniParser::parse()
{
    ...
    while (std::getline(_stream, line))
    {
        ...
        if (*line.begin() == '[' && *line.rbegin() == ']')
        {
            ...
            _tolower(_section);
            Logger::info() << "[INI] start section: `" << _section << "`" << std::endl;
            continue;
        }
        ...
        {
            Logger::info() << "[INI] malformed line: " << line << std::endl;
            continue;
        }
        ...
        Logger::info() << "[INI] string value found for property `" << name << "`: " << value << std::endl;
