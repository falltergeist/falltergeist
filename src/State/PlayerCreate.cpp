...
void PlayerCreate::init()
{
    ...
    // background
    auto background = std::make_shared<UI::Image>("art/intrface/edtrcrte.frm");
    ...
    for (unsigned int i = 0; i != 7; ++i)
    {
        ...
        _addButton(ss.str() + "_increase", std::make_shared<UI::ImageButton>(UI::ImageButton::Type::PLUS,  backgroundX+149, backgroundY+38+33*i)); // stat increase button
        _addButton(ss.str() + "_decrease", std::make_shared<UI::ImageButton>(UI::ImageButton::Type::MINUS, backgroundX+149, backgroundY+49+33*i)); // stat decrease button
    }

    _addCounter("statsPoints", std::make_shared<UI::BigCounter>(backgroundX+126, backgroundY+282)); // Free stats points counter
    ...
    for (unsigned int i = 0; i != 16; ++i)
    {
        ...
        _addImage(ss.str(), std::make_shared<UI::Image>("art/skilldex/" + imagesTraits[i] + ".frm")); // trait image
        // left column
        if (i <= 7)
        {
            _addLabel(ss.str(),  std::make_shared<UI::TextArea>(_t(MSG_TRAITS, 100 + i), backgroundX+48, backgroundY+353+13*i)); // trate label
            _addButton(ss.str(), std::make_shared<UI::ImageButton>(UI::ImageButton::Type::SKILL_TOGGLE, backgroundX+23, backgroundY+352+13*i)); // trate toggle button
        }
        //right column
        else
        {
            auto label = std::make_shared<UI::TextArea>(_t(MSG_TRAITS, 100 + i), backgroundX+169, backgroundY+353+13*(i-8));
            label->setWidth(122);
            label->setHorizontalAlign(UI::TextArea::HorizontalAlign::RIGHT);
            _addLabel(ss.str(),  label); // trate label
            _addButton(ss.str(), std::make_shared<UI::ImageButton>(UI::ImageButton::Type::SKILL_TOGGLE, backgroundX+299, backgroundY+352+13*(i-8))); // trate toggle button
        }
    }
    ...
    for (unsigned int i = 0; i != 18; ++i)
    {
        ...
        _addButton(ss.str(), std::make_shared<UI::ImageButton>(UI::ImageButton::Type::SKILL_TOGGLE, backgroundX+347, backgroundY+26+11*i));
    ...
    _addButton("options", std::make_shared<UI::ImageButton>(UI::ImageButton::Type::SMALL_RED_CIRCLE, backgroundX+345, backgroundY+454));
    ...
    _addLabel("options", std::make_shared<UI::TextArea>(_t(MSG_EDITOR, 101), backgroundX+365, backgroundY+453))->setFont(font3_b89c28ff);
    ...
    _addLabel("label_1", std::make_shared<UI::TextArea>(_t(MSG_EDITOR, 116), backgroundX+14, backgroundY+286))->setFont(font3_b89c28ff);  // char points
    _addLabel("label_2", std::make_shared<UI::TextArea>(_t(MSG_EDITOR, 139), backgroundX+50, backgroundY+326))->setFont(font3_b89c28ff);  // optinal traits
    ...
    _addLabel("label_4", std::make_shared<UI::TextArea>(_t(MSG_EDITOR, 138), backgroundX+428, backgroundY+233))->setFont(font3_b89c28ff); // tag skills
    ...
    _addImage("label_1", std::make_shared<UI::Image>("art/skilldex/generic.frm"));
    ...
    // Name change button
    _addButton("name",  std::make_shared<UI::ImageButton>(UI::ImageButton::Type::PLAYER_NAME, backgroundX+13, backgroundY+0));

    // Age change button
    _addButton("age", std::make_shared<UI::ImageButton>(UI::ImageButton::Type::PLAYER_AGE, backgroundX+156, backgroundY+0));

    //Gender change button
    _addButton("gender", std::make_shared<UI::ImageButton>(UI::ImageButton::Type::PLAYER_GENDER, backgroundX+236, backgroundY+0));
...
UI::TextArea* PlayerCreate::_addLabel(const std::string& name, std::shared_ptr<UI::TextArea> label)
{
    _labels.insert({ name, label });
    return label.get();
}

UI::ImageButton* PlayerCreate::_addButton(const std::string& name, std::shared_ptr<UI::ImageButton> button)
{
    _buttons.insert({ name, button });
    return button.get();
}

UI::BigCounter* PlayerCreate::_addCounter(const std::string& name, std::shared_ptr<UI::BigCounter> counter)
{
    _counters.insert({ name, counter });
    return counter.get();
}

UI::HiddenMask* PlayerCreate::_addMask(const std::string& name, std::shared_ptr<UI::HiddenMask> mask)
{
    ...
    for(auto it = _buttons.begin(); it != _buttons.end(); ++it)
    {
        if (it->second.get() == sender)
        {
            ...
            if (name.find("traits_") == 0)
            {
                ...
                {
                    auto state = std::make_shared<PlayerEditAlert>();
...
{
    Game::getInstance()->pushState(std::make_shared<PlayerEditAge>());
}
...
void PlayerCreate::doDone()
{
    ...
    Game::getInstance()->setState(std::make_shared<Location>());
}

void PlayerCreate::doGender()
{
    Game::getInstance()->pushState(std::make_shared<PlayerEditGender>());
}

void PlayerCreate::doName()
{
    Game::getInstance()->pushState(std::make_shared<PlayerEditName>());
}

void PlayerCreate::doOptions()
{
    Game::getInstance()->pushState(std::make_shared<PlayerCreateOptions>());
}
