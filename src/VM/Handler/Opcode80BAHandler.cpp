...
namespace Falltergeist {
    namespace VM {
        namespace Handler {
            Opcode80BA::Opcode80BA(std::shared_ptr<VM::Script> script) : OpcodeHandler(script) {
            }

            void Opcode80BA::_run() {
                Logger::debug("SCRIPT") << "[80BA] [+] int obj_is_carrying_obj_pid(GameObject* object, int PID)"
                                        << std::endl;
