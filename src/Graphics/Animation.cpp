            ...
            for (auto& direction : frm->directions())
            {
                ...
                for (unsigned int f = 0; f != frm->framesPerDirection(); ++f)
                {
                    ...
                    _texCoords.push_back(glm::vec2(
                        (float)(offsetX - 1.0) / (float)_texture->width(),
                        (float)(offsetY - 1.0) / (float)_texture->height()
                    ));
                    _texCoords.push_back(glm::vec2(
                        (float)(offsetX - 1.0) / (float)_texture->width(),
                        (float)(offsetY + srcFrame.height() + 1.0) / (float)_texture->height()
                    ));
                    _texCoords.push_back(glm::vec2(
                        (float)(offsetX + srcFrame.width() + 1.0) / (float)_texture->width(),
                        (float)(offsetY - 1.0) / (float)_texture->height()
                    ));
                    _texCoords.push_back(glm::vec2(
                        (float)(offsetX + srcFrame.width() + 1.0) / (float)_texture->width(),
