...
void Sprite::renderScaled(int x, int y, unsigned int width, unsigned int height, bool transparency, bool light, int outline, unsigned int lightValue)
{
    ...
    GL_CHECK(glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(glm::vec2), &vertices, GL_DYNAMIC_DRAW));
    ...
    GL_CHECK(glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(glm::vec2), &UV, GL_DYNAMIC_DRAW));
