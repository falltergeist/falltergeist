...
void Sprite::renderScaled(int x, int y, unsigned int width, unsigned int height, bool transparency, bool light, int outline, unsigned int lightValue)
{
    ...
    GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(vertices) * sizeof(glm::vec2), &vertices[0], GL_DYNAMIC_DRAW));
    ...
    GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(UV) * sizeof(glm::vec2), &UV[0], GL_DYNAMIC_DRAW));
